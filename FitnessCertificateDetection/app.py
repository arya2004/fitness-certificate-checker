...
@author: arya2
"""
#non-200 response handeling required
#manualy adding other distributionIDs
#differentiating between zero and O
...
#others will be done later/sone/hopefully :)

#r = requests.get('https://data.telangana.gov.in/api/1/datastore/sql?query=%5BSELECT%20%2A%20FROM%20'+'c77585eb-5808-5588-912a-5e640a7162b0'+'%5D%5BWHERE%20registrationno%20%3D%20%22'+'TS06UC4657'+'%22%5D%5BLIMIT%201%5D')

#print(str(r) == '<Response [200]>')

for distributionId in urls:
        #r = requests.get('https://data.telangana.gov.in/api/1/datastore/sql?query=%5BSELECT%20%2A%20FROM%20'+distributionId+'%5D%5BWHERE%20registrationno%20%3D%20%22'+'TS06UC4650'+'%22%5D%5BLIMIT%201%5D')
        r = requests.get('https://data.telangana.gov.in/api/1/datastore/sql?query=%5BSELECT%20%2A%20FROM%20'+distributionId+'%5D%5BWHERE%20registrationno%20%3D%20%22'+ j +'%22%5D%5BLIMIT%201%5D')
...
for i in m:
    ...
    if(parse(i["validTo"])> today):
        ...
        os.system("C:/Users/arya2/Desktop/FitnessCertificateDetection/audioOutput/hasValidFitnessCertificate/"+i["registrationNo"]+".mp3")
        print(mytext)
    else:
        tutorial1 = {"registrationNo": i["registrationNo"] ,"time": today, "hasValidFitnessCertificate": False }
        tutorial = db.tutorial
        tutorial.insert_one(tutorial1)
        mytext = i["registrationNo"] + " does not have a fitness certificte." + i["registrationNo"] + " doesn't care about enviornment"
        myobj = gTTS(text=mytext, lang=language, slow=False)
        myobj.save("audioOutput/noValidFitnessCertificate/"+i["registrationNo"]+".mp3")
        os.system("C:/Users/arya2/Desktop/FitnessCertificateDetection/audioOutput/noValidFitnessCertificate/"+i["registrationNo"]+".mp3")
        print(mytext)
#print(i["validTo"])
#C:/Users/arya2/Desktop/FitnessCertificateDetection/audioOutput/noValidFitnessCertificate/TS06UC4650.mp3   